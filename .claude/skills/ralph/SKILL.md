...
allowed-tools: "Read Write Edit Glob Grep Bash(mvn *) Bash(./scripts/*) Bash(mkdir *) Bash(cp *) Bash(mv *) Bash(ls *) Bash(git *) Bash(cat *) Bash(echo *) Agent"
---
...
3. 初始化 progress.md（如尚未存在）：
   ```bash
   ./scripts/ralph-init.sh docs/exec-plan/active/${TASK_ID}
   ```
4. 显示当前状态（已完成/未完成任务数 + 下一个待办）
5. 输出终端命令供用户执行：
   ```
   ./scripts/ralph-loop.sh ${AGENT} docs/exec-plan/active/${TASK_ID}
   ```
