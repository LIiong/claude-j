...
**Ralph 自身禁止事项**：
- 不直接编辑业务代码、测试、POM、YAML、schema、Mapper XML。
- 不因 Build/Debug 阻塞而自己下场修复；只能整理证据并重新调度子 Agent。
- 若 `@dev` 在 Build 阶段连续 3 轮仍未收敛，必须先升级给 `@architect` 做架构复评，再回到 `@dev`。
- 若同一阻塞已消耗 2 轮 `@architect` 复评/修复方案后仍无解，必须终止任务推进并向用户报告。
...
- 若 `@dev` 连续 3 轮仍未收敛（根因反复、修复目标漂移、验证始终卡在不同装配/边界问题）→ 停止直接继续 Build，调度 `@architect` 做架构复评，再将复评结论交回 `@dev`
- 若同一阻塞已消耗 2 轮 `@architect` 复评/修复方案后仍未解决 → 终止任务推进，反馈用户，并要求把问题与尝试记录进 `dev-log.md`

若 `--dev-only` → 输出完成摘要，流程结束。

### Build 阻塞升级规则
- 若 `@dev` 在 Build 阶段连续 3 轮仍未收敛，Ralph 必须停止“继续让 @dev 硬修”的循环。
- Ralph 需要先整理证据：失败命令、错误摘要、已尝试修复、当前最小阻塞点。
- 然后调度 `@architect` 做一次**架构复评**，判断是否存在：设计遗漏、边界错误、模块依赖错误、Bean 装配错误、测试装配策略错误、阶段拆分错误。
- `@architect` 复评后给出结论与约束，Ralph 再将复评结论交回 `@dev` 继续 Build。
- 若同一阻塞已消耗 2 轮 `@architect` 复评/修复方案后仍无解，Ralph 必须终止任务推进，向用户报告问题清单，并要求把阻塞经过、复评结论、已尝试方案写入 `dev-log.md`。
- 在此升级点之前，Ralph 不得自己编辑实现文件或直接接管 Debug。
