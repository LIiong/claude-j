...
> **状态**：半生产级升级路径 · 首次发布 2026-04-17 · 已完成 P0 11/12（剩余：B2）· 已完成 P1-必做 4/6（剩余：D1、E6）
...
| # | 任务 | 优先级 | 工作量 | 价值 | 完成标志 |
|:---:|---|:---:|:---:|---|---|
| A1 | **补齐 Order ↔ Coupon 集成** | 🔴 P0 | 中 | 当前 Coupon 作为独立聚合存在但 Order 创建时 `couponId` 被忽略，是教学漏洞 | ✅ 已完成（task 015）创建订单能正确应用优惠、Coupon 标记 used、订单金额反映折扣 |
| A2 | **补齐 Order 状态机**（ship / deliver / refund） | 🔴 P0 | 中 | 仅 CREATED→PAID→CANCELLED，缺真实电商核心状态 | ✅ 已完成（task 016）SHIPPED / DELIVERED / REFUNDED 状态 + API + 对应不变量测试 |
| A3 | **分页 & 排序**（所有 list 接口） | 🔴 P0 | 小 | 当前全表返回，真生产会 OOM | ✅ 已完成（task 017）基于分页请求/响应改造所有 listXxx 端点 |
| A4 | **库存聚合（Inventory）** | 🟡 P1-必做 | 大 | 电商缺库存等于没骨架；当前下单不扣库存 | ✅ 已完成（task 023）新建 `inventory` 聚合，订单创建时预占、支付时扣减、取消时回滚 |
| A5 | **订单项与产品聚合拆分** | 🟡 P1-必做 | 大 | 当前 `OrderItem` 手填 productId/price，无 Product 聚合 | ✅ 已完成（task 021）新建 `product` 聚合（SKU / 定价 / 上下架） |
| A6 | **支付聚合 + 支付状态回调** | 🟡 P1-必做 | 大 | 当前 `payOrder()` 只是改状态，无支付方/三方对接模型 | ✅ 已完成（task 024）新建 `payment` 聚合，抽象 PSP 接口（可 mock）+ 回调 webhook 端点 |
...
| A11 | **领域事件发布 + 跨聚合最终一致性** | 🟡 P1-必做 | 中 | 当前跨聚合靠 Application 直接调用，缺事件载体 | ✅ 已完成（task 025）Spring 领域事件驱动订单支付成功 → 库存扣减 / 优惠券核销 |
...
| # | 任务 | 优先级 | 工作量 | 价值 | 完成标志 |
|:---:|---|:---:|:---:|---|---|
| B1 | **Spring Boot Actuator + 健康检查** | 🔴 P0 | 小 | K8s 就绪/存活探针的基础 | ✅ 已完成（task 018）`/actuator/health`, `/actuator/info` 开放；liveness/readiness 分离 |
| B2 | **Micrometer + Prometheus 指标端点** | 🔴 P0 | 小 | 所有 JVM/HTTP/DB 指标标准化 | `/actuator/prometheus` + 关键业务指标（下单量、失败率） |
| B3 | **结构化日志 + MDC + TraceId** | 🔴 P0 | 中 | 当前只有 logback 默认 | ✅ 已完成（task 019）JSON 日志 + MDC TraceId + `X-Request-Id` 回传 |
...
| # | 任务 | 优先级 | 工作量 | 价值 | 完成标志 |
|:---:|---|:---:|:---:|---|---|
| C1 | **Spring Security + 授权** | 🔴 P0 | 中 | JWT 已有认证但无授权；登录后无任何访问控制，严重漏洞 | ✅ 已完成（task 022）JwtAuthenticationFilter + `@PreAuthorize` + 角色模型 |
| C2 | **密钥外置化** | 🔴 P0 | 小 | 现在 JWT secret 在 application.yml 明文 | ✅ 已完成（task 010）环境变量读取 + 本地开发默认值 + secrets 文档 |
| C3 | **CORS 配置** | 🔴 P0 | 极小 | 前后端分离必须 | ✅ 已完成（task 026）`CorsConfigurationSource` bean + 可配置白名单 |
| C4 | **Flyway 数据库迁移** | 🔴 P0 | 小 | 当前手写 schema.sql，无版本管理 | ✅ 已完成（task 011）`db/migration/V1__init.sql` 起步，CI 校验 migration 可从零重放 |
...
| # | 任务 | 优先级 | 工作量 | 价值 | 完成标志 |
|:---:|---|:---:|:---:|---|---|
| E1 | **OpenAPI / Swagger 文档**（springdoc-openapi） | 🔴 P0 | 小 | 对接方必备 | ✅ 已完成（task 020）`/swagger-ui.html` + `/v3/api-docs`；核心 Controller 已标注 |
| E2 | **API 版本策略文档** | 🟢 P2 | 极小 | 已有 `/v1` 前缀但策略未文档化 | `docs/standards/api-versioning.md` |
| E3 | **staging / prod 多环境配置** | 🔴 P0 | 小 | 当前仅 dev | ✅ 已完成（task 012）application-staging.yml + application-prod.yml + `docs/ops/profiles.md` |
...
**理由**：当前 P0 仅剩可观测基线里的 Prometheus 指标端点；安全底线里的 CORS 已完成，其余 P0 也已收口。
...
构成 "半生产电商骨架" 的剩余必要条件：消息队列 + Testcontainers 替代 H2。
...
| **4** | **B1 Actuator + B2 Prometheus** | 可观测 | Health 已完成，当前只剩指标端点与业务 metrics 收尾 | 无 |
| **5** | **B3 结构化日志 + TraceId** | 可观测 | 已完成 requestId 贯穿，生产问题可追查 | 无 |
| **6** | **C1 Spring Security + 授权** | 安全 | 已完成 JWT Filter + `@PreAuthorize` 授权基线 | C2 |
| **7** | **E1 OpenAPI 文档** | API | 已完成 Swagger UI / OpenAPI JSON 暴露 | 无 |
| **8** | **A3 分页** | 业务 | 已完成所有 list 接口分页改造，避免全表返回 | 无 |
| **9** | **A1 Coupon↔Order 集成** | 业务 | 已完成订单用券、核销、取消回滚 | 无 |
| **10** | **A2 订单完整状态机**（SHIPPED/DELIVERED/REFUNDED） | 业务 | 已完成发货/送达/退款状态机链路 | A1 |

**做完 Top 10 之后**，当前进入批 3 收尾阶段：优先完成 `D1` 与 `E6`，并将 `B2` 作为剩余 P0 收尾项完成。
...
| 批次 | 范围 | 项数 | 工作量估算 |
|:---:|---|:---:|:---:|
| 批 1 | P0 基础设施：E3 → C2 → C3 → C4 → B1 → B2 → B3 → E1 → C1 | 9 | 已完成 8/9（剩余：B2） |
| 批 2 | P0 业务修补：A3 → A1 → A2 | 3 | ✅ 已完成 |
| 批 3 | P1 必做 电商骨架：A5 → A4 → A6 → A11 → D1 → E6 | 6 | 已完成 4/6（剩余：D1、E6） |
...
| 日期 | 变更 | 触发 |
|---|---|---|
| 2026-04-28 | 根据已归档任务回填完成状态：P0 已完成 A1/A2/A3/B1/B3/C1/C2/C3/C4/E1/E3，P1-必做已完成 A4/A5/A6/A11；当前剩余 P0 为 B2，剩余 P1-必做为 D1/E6。 | 归档任务复盘 |
