...
| 5 | Domain: OrderPaidEvent + 测试 | dev | 待办 | |
| 6 | Domain: OrderCancelledEvent + 测试 | dev | 待办 | |
| 7 | Domain: PaymentSuccessEvent + 测试 | dev | 待办 | |
| 8 | Domain: PaymentRefundedEvent + 测试 | dev | 待办 | |
| 9 | Infrastructure: SpringDomainEventPublisher + 测试 | dev | 待办 | |
| 10 | Infrastructure: InventoryEventListener + 测试 | dev | 待办 | |
| 11 | Application: OrderApplicationService 改造（发布事件）+ 更新测试 | dev | 待办 | |
| 12 | Application: PaymentApplicationService 改造（发布事件）+ 更新测试 | dev | 待办 | |
| 13 | 全量 mvn test | dev | 待办 | |
| 14 | 全量 mvn checkstyle:check | dev | 待办 | |
| 15 | ./scripts/entropy-check.sh | dev | 待办 | |
| 16 | QA: 测试用例设计 | qa | 待办 | |
| 17 | QA: 验收测试 + 代码审查 | qa | 待办 | |
| 18 | QA: 事件流程集成测试 | qa | 待办 | |
...
### 1.5 Domain 事件 OrderPaidEvent
- **文件**：`claude-j-domain/src/main/java/com/claudej/domain/order/event/OrderPaidEvent.java`
- **测试**：`claude-j-domain/src/test/java/com/claudej/domain/order/event/OrderPaidEventTest.java`
- **骨架**：类似 OrderCreatedEvent
- **验证命令**：`mvn test -pl claude-j-domain -Dtest=OrderPaidEventTest`
- **预期输出**：`Tests run: X, Failures: 0, Errors: 0`
- **commit**：`feat(domain): 订单支付事件 OrderPaidEvent`

### 1.6 Domain 事件 OrderCancelledEvent
...
### 1.7 Domain 事件 PaymentSuccessEvent
...
### 1.8 Domain 事件 PaymentRefundedEvent
...
- **改造内容**：
  ...
  4. payOrder() 移除 deductStockForOrder()，改为发布 OrderPaidEvent
