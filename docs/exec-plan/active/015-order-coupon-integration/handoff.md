...
1. **Money 类缺少 subtract 方法** — 已在评审意见中建议补充
2. **CouponStatus 需要扩展回滚转换方法** — 已在评审意见中建议补充

以上两项应在 Build 阶段作为前置任务完成。
...
- 状态：approved
- 说明：015-order-coupon-integration 架构评审通过，设计符合六边形架构规范。发现 2 项需要在 Build 阶段补充的方法（Money.subtract, CouponStatus.toAvailable）。
