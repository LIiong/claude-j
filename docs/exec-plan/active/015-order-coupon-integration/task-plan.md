...
| # | 任务 | 负责人 | 状态 | 备注 |
|---|------|--------|------|------|
| 1 | Domain: Order 聚合添加 coupon 相关字段和方法 | dev | 待办 | 含 applyCoupon, removeCoupon |
| 2 | Domain: Coupon 聚合添加 unuse() 方法 | dev | 待办 | 状态回滚逻辑 |
| 3 | Domain: Order/Coupon 领域层单元测试 | dev | 待办 | JUnit 5 + AssertJ |
| 4 | Application: 扩展 Command 和 DTO | dev | 待办 | couponId 字段 |
| 5 | Application: 扩展 OrderAssembler | dev | 待办 | 映射新字段 |
| 6 | Application: OrderApplicationService 优惠券集成 | dev | 待办 | 验证/计算/核销/回滚 |
| 7 | Application: 应用层单元测试（Mockito） | dev | 待办 | Mock CouponRepository |
| 8 | Infrastructure: OrderDO 扩展字段 | dev | 待办 | coupon_id, discount_amount, final_amount |
| 9 | Infrastructure: OrderConverter 更新 | dev | 待办 | 往返映射 |
| 10 | Infrastructure: Repository 实现更新 | dev | 待办 | 保存新字段 |
| 11 | Infrastructure: 持久化层集成测试（H2） | dev | 待办 | DO↔Domain 往返验证 |
| 12 | Start: schema.sql DDL 更新 | dev | 待办 | t_order 表结构 |
| 13 | 全量 mvn test 验证 | dev | 待办 | 含 ArchUnit |
| 14 | 代码风格检查 | dev | 待办 | checkstyle |
| 15 | 熵检查 | dev | 待办 | entropy-check.sh |
| 16 | QA: 测试用例设计 | qa | 待办 | |
| 17 | QA: 验收测试 + 代码审查 | qa | 待办 | |
| 18 | QA: 集成测试（完整流程） | qa | 待办 | 下单→支付→取消 |
