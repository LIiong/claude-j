...
run "cp    '${SOURCE_ROOT}/scripts/verify-portable.sh'     '${TARGET_DIR}/scripts/verify-portable.sh' 2>/dev/null || true"
...
echo ""
echo "下一步："
echo "  1. ./scripts/verify-portable.sh ${TARGET_DIR}    # 自检产物完整性（预期 0 FAIL）"
echo "  2. cd ${TARGET_DIR}"
echo "  3. 查看并调整 CLAUDE.md"
echo "  4. 按 PORTING.md 完成 L1/L2/L3 守护适配"
echo "  5. 初始化构建骨架（${BUILD_TOOL}）"
echo "  6. 在 Claude Code 中测试：/ralph 001-hello-world 实现一个 hello 端点"
echo ""
